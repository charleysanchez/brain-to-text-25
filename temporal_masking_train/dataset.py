...
class BrainToTextDataset(Dataset):
    ...
    def __init__(
        ...
        '''
        trial_indices:  (dict)       - dictionary with day numbers as keys and lists of trial indices as values
        ...
        feature_subset  ([int])      - list of neural feature indices that should be the only features included in the neural data 
         '''
            ...
            with h5py.File(self.trial_indices[d]['session_path'], 'r') as f:
                ...
                for t in index[d]:
                    ...
                    try: 
                        ...
                        batch['day_indices'].append(int(d)) # day index of each trial - required for the day specific layers
                        batch['block_nums'].append(g.attrs['block_num'])
                        batch['trial_nums'].append(g.attrs['trial_num'])

                    except Exception as e:
                        print(f'Error loading trial {t} from session {self.trial_indices[d]["session_path"]}: {e}')
                        continue
    ...
    def create_batch_index_train(self):
        ...
        if self.must_include_days is not None:
            non_must_include_days = [d for d in self.trial_indices.keys() if d not in self.must_include_days]
        ...
        for batch_idx in range(self.n_batches):
            ...
            # Otherwise we will select random days without replacement
                ...
                days = np.random.choice(list(self.trial_indices.keys()), size = self.days_per_batch, replace = False)
            ...
            for d in days:
                ...
                trial_idxs = np.random.choice(self.trial_indices[d]['trials'], size = num_trials, replace = True)
                batch[d] = trial_idxs
...
def train_test_split_indices(file_paths, test_percentage = 0.1, seed = -1, bad_trials_dict = None):
    '''
